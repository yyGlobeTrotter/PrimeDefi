...
contract Deal {
    ...
    */
     ...
     */
    function editInvestorDetailsPostNewIssuance(
        string memory _ISIN,
        Investor storage _investor
    ) internal {
        ...
        public
    {
        ...
        require(
            deal.isOfferLive,
            "DEAL:: createTokenMetaData: Offer is not live"
        );
        require(
    ...
    function closeDealOffering(string memory _ISIN) public returns (bool) {
        ...
        if (!toLaunch) {
            ...
            uint256 ratioInDecimal = safeDiv(
                safeMul(finalSize, 10**feeRateDecimal),
                deal.totalInvestorBid
            );
            ...
            for (uint256 i = 0; i < deal.investors.length; i++) {
                ...
                if (deal.totalInvestorBid > finalSize) {
                    ...
                    deal.totalEscrowAmount = safeAdd(
                        ...
                        safeDiv(
                            safeMul(
                                safeDiv(deal.escrowRatio, 10**feeRateDecimal),
                                finalAllocation
                            ),
                            10**percentageDecimal
                        )
                    );
    ...
    function mintToken(string memory _ISIN, address _issuerAddr) internal {
        ...
        DealIssuance storage deal = deals[_ISIN];
        //require(deal.token.name != "", "DEAL:: mintToken: Token name is not set");
