 ...
 * @dev Implementation for USDC stablecoin yet to be finalized. Might need to create our own fake USDC for testing purposes
 ...
 * @dev To implement existing issuer/investor/token name...checking at all relevant places
...
contract Deal {
    ...
    /// PLACEHOLDER for Events - Publicize important actions to external listeners
    event LogCreateIssuer(
        address indexed _issuer,
        string _name,
        string _creditRating
    );
    event LogEditIssuerMetaData(
        address indexed _issuer,
        string _name,
        string _creditRating
    );
    ...
    event LogCreateDealIssuance(
        ...
        uint256 _offerCloseTime,
        uint256 _term,
        uint256 _interestRate,
        uint256[] _interestPaymentDates,
        uint256 _upfrontFee
    ...
    event LogEditDealIssuance(
        ...
        uint256 _newOfferCloseTime,
        uint256 _newTerm,
        uint256 _newInterestRate,
        uint256[] _newInterestPaymentDates,
        uint256 _newUpfrontFee,
        uint256 _newEscrowRatio
    );

    event LogEditTokenMetaData(
        address indexed _issuer,
        string indexed _ISIN,
        string _newName
    );
    event LogCreateToken(
        address indexed _issuer,
        string indexed _ISIN,
        string _name,
        string _symbol,
    ...
    uint256 public globalFee; // this is the fee our dapp charges; Assume a flat 5% of total issue size for now
    ...
    mapping(string => bool) isExistingISIN; // map out ISIN to whether ISIN's been used in the dapp
    ...
    modifier isOfferClosed(string memory _ISIN) {
        require(
            deals[_ISIN].isOfferClosed,
            "Issuance offer period is NOT closed yet"
        );
        _;
    }
    ...
    */
     ...
     */
    function createIssuer(string memory _name, string memory _creditRating)
        public
    {
     ...
     */
    function editIssuerMetaData(
        address _addr,
        string memory _newName,
        string memory _newCreditRating
    ) public {
        require(
            msg.sender == _addr,
            "DEAL:: editIssuerDetails: Caller is NOT the issuer"
        );
    ...
    /**
     * @notice Edit issuer issuance details after new deal offer is closed and set to launch
     */
    function editIssuerDetailsPostNewIssuance(
        string memory _ISIN,
        address _addr
    ) internal {
        ...
        _issuer.totalIssuedAmount = safeAdd(
            _issuer.totalIssuedAmount,
            deal.finalSize
        );
        _issuer.totalEscrowFundLockedIn = safeAdd(
            _issuer.totalEscrowFundLockedIn,
            deal.totalEscrowAmount
        );
        _issuer.totalAvailBalance = safeSub(
            _issuer.totalIssuedAmount,
            _issuer.totalEscrowFundLockedIn
        );
     ...
     */
    function getAllIssuerDetailsByAddr(address _addr)
        public
        view
        returns (
            string memory name,
        ...
        )
    {
        require(
            msg.sender == _addr,
            "DEAL:: getAllIssuerDetailsByAddr: Caller is NOT the issuer"
        );
     ...
     */
    function getIssuerMetaDataByISIN(string memory _ISIN)
        public
        view
        returns (string memory issuerName, string memory creditRating)
    {
        return (
            issuers[deals[_ISIN].issuer].name,
            issuers[deals[_ISIN].issuer].creditRating
        );
    }
     ...
     */
    function getAllInvestorDetails(address _addr)
        public
        view
        returns (
        ...
        )
    {
        require(
            msg.sender == _addr,
            "DEAL:: getAllInvestorDetails: Caller is NOT the investor"
        );

        return (
            investors[_addr].dealsBidded,
            investors[_addr].totalBalance,
            investors[_addr].totalLockedInBid,
            investors[_addr].availBalance
        );
    }
     ...
     */
    function getIssuanceStateNameRateDatesDetailsByISIN(string memory _ISIN)
        public
        view
        returns (
            State state,
            ...
            uint256 offerCloseTime,
            uint256 term,
            uint256 interestRate,
            uint256[] memory interestPaymentDates
        )
    {
        DealIssuance storage deal = deals[_ISIN];
        return (
     ...
     * @dev split DealIssuance getter functions into a few separate ones, otherwise stack too deep :(
     */
    function getIssuanceSizePriceTotalBidDetailsByISIN(string memory _ISIN)
        public
        view
        returns (
            uint256 initSize,
    ...
    function editDealIssuance(
        ...
        uint256 _newTerm,
        uint256 _newInterestRate,
        uint256[] memory _newInterestPaymentDates,
        uint256 _newUpfrontFee,
        uint256 _newEscrowRatio
    ) public {
        require(
            msg.sender == deals[_ISIN].issuer,
            "DEAL:: editDealIssuance: Caller is NOT the issuer"
        );
        require(
            deals[_ISIN].isOfferLive,
            "DEAL:: editDealIssuance: You cannot edit an issuance when it is NOT live"
        );
     ...
     */
    function createTokenMetaData(string memory _ISIN, string memory _name)
        public
    {
        DealIssuance storage deal = deals[_ISIN];
        require(
            msg.sender == deal.issuer,
            "DEAL:: createTokenMetaData: Caller is NOT the issuer"
        );
        //require(!deal.isOfferClosed, "DEAL:: setTokenDetails: Offer period is closed ");
     ...
     */
    function getTokenDetails(string memory _ISIN)
        public
        view
        returns (
    ...
    {
        return (
            deals[_ISIN].token.name,
            deals[_ISIN].token.symbol,
            deals[_ISIN].token.totalSupply
        );
    }
     ...
     */
    function editTokenMetaData(string memory _ISIN, string memory _newName)
        public
    {
        require(
            msg.sender == deals[_ISIN].issuer,
            "DEAL:: editTokenDetails: Caller is NOT the issuer"
        );
        require(
            !deals[_ISIN].isOfferClosed,
            "DEAL:: editTokenMetaData: Offer period is already closed "
        );
    ...
    function closeDealOffering(string memory _ISIN) public returns (bool) {
        ...
        require(
            msg.sender == deals[_ISIN].issuer,
            "DEAL:: closeDealOffering: Caller is NOT the issuer"
        );
        require(
            deals[_ISIN].isOfferLive,
            "DEAL:: closeDealOffering: Offer cannot be closed because it is NOT live"
        );

        DealIssuance storage deal = deals[_ISIN];
        Issuer storage _issuer = issuers[deal.issuer];
        ...
        if (!toLaunch) {
            ...
            uint256 ratio = safeDiv(finalSize, deal.totalInvestorBid);

            for (uint256 i = 0; i < deal.investors.length; i++) {
                address investorAddr = deal.investors[i];

                if (finalSize < deal.totalInvestorBid) {
                    uint256 finalAllocation = safeMul(
                        ratio,
                        investors[investorAddr].lockedInBid[_ISIN]
                    );
                    deal.totalEscrowAmount = safeAdd(
                        deal.totalEscrowAmount,
                        safeMul(safeDiv(deal.escrowRatio, 100), finalAllocation)
                    );

                    investors[investorAddr].lockedInBid[
                        _ISIN
                    ] = finalAllocation;

                    // adjust allocated investment amount for each investor
                    adjustInvestorAllocation(
                        _ISIN,
                        finalAllocation,
                        investorAddr
                    );
                ...
                investors[investorAddr].isOfferClosed[_ISIN] = true;
                investors[investorAddr].isBidSuccessful[_ISIN] = true;
            }

            deal.totalInvestorBid = safeMul(ratio, deal.totalInvestorBid);
            deal.state = State.ISSUED;

            editIssuerDetailsPostNewIssuance(_ISIN, deal.issuer);

            emit LogIssueNewDeal(
                deal.issuer,
                _ISIN,
                deal.token.tokenAddr,
                finalSize,
                deal.investors
            );
     ...
     */
    function assessDealOfferingFinalStatus(string memory _ISIN)
        internal
        returns (bool)
    {
        if (deals[_ISIN].totalInvestorBid >= deals[_ISIN].minSize) {
            deals[_ISIN].finalSize = Math.min(
                deals[_ISIN].totalInvestorBid,
                deals[_ISIN].initSize
            );
            return true;
        } else {
            return false;
     ...
     */
    function adjustInvestorAllocation(
        string memory _ISIN,
        uint256 _finalAllocation,
        address _investorAddr
    ) internal {
        Investor storage investor = investors[_investorAddr];

        investor.totalBalance = safeSub(
            investor.totalBalance,
            _finalAllocation
        );
        investor.totalLockedInBid = safeSub(
    ...
    function mintToken(string memory _ISIN, address _issuerAddr) internal {
        require(
            msg.sender == _issuerAddr,
            "DEAL:: mintToken: Caller is NOT the issuer"
        );
        ...
        address addr = factory.createToken(_name, _symbol, _totalSupply);
        manager.setTokenOwner(addr, address(this));

        DealToken(addr).increaseAllowance(address(manager), _totalSupply);

        tracker_0x_addresses[_issuerAddr].push() = addr;
        tokenCount++;
    ...
    function investorMakeBid(string memory _ISIN, uint256 _bidAmount) public {
        ...
        require(
            ...
            "DEAL:: investorMakeBid: you have already made a bid"
        );
        require(
            _bidAmount <= investor.totalBalance,
            "DEAL:: investorMakeBid: insuffient fund"
        );
        ...
        investor.availBalance = safeSub(investor.totalBalance, _bidAmount);
    ...
    function investorEditBid(string memory _ISIN, address _addr) external {
        //......
    }

    function investorCancelBid(string memory _ISIN, address _addr) external {
        //......
    }

    function investorWithdraw(address _addr, uint256 _amount) external {
        //......
    }
